...
def vectorized_clean_series(s: pd.Series):
    ...
    # 3. Apply negation handling before tokenization and stopword removal
    s = s.apply(handle_negation)  # <------ Insert negation handling here

    # 4. Final structural cleanup (Punctuation and whitespace)
    s = s.str.replace(r'[^a-z_\s]', ' ', regex=True)  # Keep _ for _NEG
    s = s.str.replace(r'\s+', ' ', regex=True).str.strip()

    # 5. Tokenize and remove augmented stopwords (including underscore tokens)
    s = s.apply(lambda x: ' '.join([word for word in word_tokenize(x) if word not in stop_words and word]))
